...
bool C610::send_message() {
  ...
  bool success = true;
  if (!can_write(can, CANMessage(0x200, buf, 8))) {
    // printf("[c610]can't send(1~4)\n");
    success = false;
  }
  osDelay(1); // 少し待ってから次の送信（必要に応じて調整）
  if (!can_write(can, CANMessage(0x1FF, buf + 8, 8))) { // bufの後半部分を送信
    // printf("[c610] can't send(5~8)\n");
    success = false;
  }
...
void C610::stop() { std::fill(send_power, send_power + 8, 0); }
...
void C610::param_update() {
  CANMessage msg;
  // ★ whileに変更し、バッファに溜まっているメッセージを全て読み切る
  while (can_read(can, msg)) {
    if (msg.format == CANMessage::CANStandard &&
        msg.type == CANMessage::CANData && msg.len == 8) {
...
void C610::can_reset() { /* HAL_CAN_Reset? Not typical, skip or reinteg */; }
...
int16_t C610::get_send_power(int id) { return send_power[id - 1]; }
