...
bool C610::send_message()
{
    ...
    if (!can_write(can, CANMessage(0x1FF, buf + 8, 8)))
    { // bufの後半部分を送信
...
void C610::stop()
{
    std::fill(send_power, send_power + 8, 0);
}
...
{
    CANMessage msg;
    if (can_read(can, msg) && msg.format == CANMessage::CANStandard && msg.type == CANMessage::CANData && msg.len == 8)
    {
...
void C610::can_reset()
{
    /* HAL_CAN_Reset? Not typical, skip or reinteg */;
}
